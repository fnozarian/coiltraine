...
class CoILEnsembleAgent(object):
    def __init__(self, checkpoints, town_name, carla_version='0.84'):
        # Set the carla version that is going to be used by the interface
        self._carla_version = carla_version
        self.checkpoints = checkpoints  # We save the checkpoint for some interesting future use.
        self.first_iter = True

        self._models = []
        for i, checkpoint in enumerate(self.checkpoints):
            print("loading model %/%".format(i, len(self._models)))
    ...
    def run_step(self, measurements, sensor_data, directions, target):
        ...
        for model in self._models:
            means, log_vars = model.forward_branch(self._process_sensors(sensor_data), norm_speed,
                                                  directions_tensor)
            models_outputs.append(means)
        ensemble_output = torch.mean(torch.cat(models_outputs), dim=0)

        steer, throttle, brake = self._process_model_outputs(ensemble_output)
        ...
        if g_conf.USE_ORACLE:
            _, control.throttle, control.brake = self._get_oracle_prediction(
                measurements, target)
        checkpoints_iterations = [checkpoint['iteration'] for checkpoint in self.checkpoints]
        if self.first_iter:
            coil_logger.add_message('Iterating', {"Checkpoints": checkpoints_iterations,
                                                  'Agent': str(steer)},
                                    checkpoints_iterations[-1])
    ...
    def get_attentions(self, layers=None):
        ...
        all_layers = self._models[-1].get_perception_layers(self.latest_image_tensor)
